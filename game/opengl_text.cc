...
namespace
{
	PangoAlignment parseAlignment(std::string const &fontalign)
	{
		...
		if (fontalign == "center" || fontalign == "middle")
...
namespace
{
	void parseTheme(fs::path const &themeFile, TextStyle &_theme, float &_width, float &_height, float &_x, float &_y, SvgTxtTheme::Align &_align)
	{
		...
		if (n.empty())
			throw std::runtime_error("Unable to find text theme info width in " + themeFile.string());
		...
		if (n.empty())
			throw std::runtime_error("Unable to find text theme info height in " + themeFile.string());
		...
		if (n.empty())
			throw std::runtime_error("Unable to find text theme info style in " + themeFile.string());
		...
		while (std::getline(iss, token, ';'))
		{
			...
			else if (token == "font-family")
				std::getline(iss2, _theme.fontfamily);
			else if (token == "font-style")
				std::getline(iss2, _theme.fontstyle);
			else if (token == "font-weight")
				std::getline(iss2, _theme.fontweight);
			else if (token == "stroke-width")
				iss2 >> _theme.stroke_width;
			else if (token == "stroke-opacity")
				iss2 >> _theme.stroke_col.a;
			else if (token == "stroke-linejoin")
				iss2 >> _theme.stroke_linejoin;
			else if (token == "stroke-miterlimit")
				iss2 >> _theme.stroke_miterlimit;
			else if (token == "stroke-linecap")
				iss2 >> _theme.stroke_linecap;
			else if (token == "fill-opacity")
				iss2 >> _theme.fill_col.a;
			else if (token == "fill")
				iss2 >> _theme.fill_col;
			else if (token == "stroke")
				iss2 >> _theme.stroke_col;
			else if (token == "text-anchor")
			{
				...
				if (value == "start")
					_align = SvgTxtTheme::Align::LEFT;
				else if (value == "middle")
					_align = SvgTxtTheme::Align::CENTER;
				else if (value == "end")
					_align = SvgTxtTheme::Align::RIGHT;
			}
		}
		// Parse x and y attributes
		n = dom.get_document()->get_root_node()->find("/svg:svg//svg:text/@x", nsmap);
		if (n.empty())
			throw std::runtime_error("Unable to find text theme info x in " + themeFile.string());
		...
		if (n.empty())
			throw std::runtime_error("Unable to find text theme info y in " + themeFile.string());
...
void SvgTxtTheme::draw(std::vector<TZoomText> &_text, bool lyrics)
{
	...
	if (m_align == Align::CENTER)
		position_x -= 0.5f * m_texture_width;
	if (m_align == Align::RIGHT)
		position_x -= m_texture_width;
